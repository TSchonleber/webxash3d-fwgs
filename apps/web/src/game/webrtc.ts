// Ported from examples/react-typescript-cs16-webrtc/src/webrtc.ts.
// Connects the Xash3D engine to a cs-web-server over WebRTC data channels.
import { Xash3D, Net } from "xash3d-fwgs";
import type { Packet, Xash3DOptions } from "xash3d-fwgs";
...
export class Xash3DWebRTC extends Xash3D {
  ...
  initConnection(stream: MediaStream) {
    ...
    this.peer.onicecandidate = (e) => {
      ...
      this.ws!.send(
        JSON.stringify({ event: "candidate", data: JSON.stringify(e.candidate.toJSON()) }),
      );
    };
    ...
    this.peer.ondatachannel = (e) => {
      if (e.channel.label === "write") {
        e.channel.onmessage = (ee) => {
          const packet: Packet = { ip: [127, 0, 0, 1], port: 8080, data: ee.data };
  ...
  sendto(packet: Packet) {
    ...
    this.channel.send(packet.data as unknown as ArrayBuffer);
  }
}

...
import { computeAwards, computeScoreWeightedAwards } from "./payout";
...
/**
 * Settle a full UTC day: rank the day's Top-10 by efficiency skill rating, screen
 * cheats, apply the hold-eligibility gate, then split the budget SCORE-WEIGHTED
 * (each winner earns in proportion to their rating). Builds the claim tree.
 */
export async function settleDay(matches: MatchResult[], ctx: SettleCtx): Promise<Settlement> {
  const flagged = new Set<string>();
  for (const m of matches) for (const p of m.players ?? []) if (screenPlayer(p).suspicious) flagged.add(p.wallet);

  const board = rankDaily(matches, 10).filter((e) => !flagged.has(e.wallet));

  const eligible = [];
  for (const e of board) if (await ctx.isEligible(e.wallet)) eligible.push(e);

  const top = eligible.slice(0, 10);
  const { awards } = computeScoreWeightedAwards(top, { vaultLamports: ctx.vaultLamports, budgetBps: ctx.budgetBps });
